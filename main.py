...
def spusteni():
    ...
    if len(sys.argv) != 3:
        print(
            "Chyba pri zadani: Ke spusteni skriptu musite zadat presne dva argumenty, url a vystup (bez pripony .csv). V prikladu ve spicatych zavorkach")
...
def vyber_ucelek(url: str, vystup: str) -> bs:
    ...
    Napr.:  https://www.volby.cz/pls/ps2017nss/ps32?xjazyk=CZ&xkraj=2&xnumnuts=2102 kladno
...
def najdi_odkazy_obci(odpoved_s_obcemi: bs) -> list:
    ...
    td_tags = odpoved_s_obcemi.find_all("td", {
        "class": "cislo"})  # tag td s argumentem class o hodnote cislo jako zdroj odkazu
    for a_href in td_tags:  # cyklus ktery vyhledava fragmenty
        tag = a_href.find("a")
        fragmenty_odkazu.append(
            tag.attrs.get("href", "nejsou data"))  # najde odkaz nebo vypise, ze odkaz neboli data nejsou
...
def volici_v_obci(bs_obec: str) -> int:
    ...
    td_tag = bs_obec.find("td", class_="cislo",
                          headers="sa2")  # tagy s td argumentem a spravnou class a sa jako zdroj poctu volicu
    if td_tag:
...
def obalky_v_obci(bs_obec: str) -> int:
    ...
    td_tag = bs_obec.find("td", class_="cislo",
                          headers="sa3")  # tagy s td argumentem a spravnou class a sa jako zdroj poctu obalek
    if td_tag:
...
def hlasy_v_obci(bs_obec: str) -> int:
    ...
    td_tag = bs_obec.find("td", class_="cislo",
                          headers="sa6")  # tagy s td argumentem a spravnou class a sa jako zdroj poctu hlasu
    if td_tag:
